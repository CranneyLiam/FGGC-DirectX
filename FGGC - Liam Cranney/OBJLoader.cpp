...
bool OBJLoader::FindSimilarVertex(const SimpleVertex2& vertex, std::map<SimpleVertex2, unsigned short>& vertToIndexMap, unsigned short& index)
{
							  ...
							  std::vector<XMFLOAT3>& outNormals)
{
	...
	std::map<SimpleVertex2, unsigned short> vertToIndexMap;

	std::pair<SimpleVertex2, unsigned short> pair;

	int numVertices = inVertices.size();
	
	for(int i = 0; i < numVertices; ++i) //For each vertex
	{
		SimpleVertex2 vertex = {inVertices[i], inNormals[i],  inTexCoords[i]}; 
...
MeshData OBJLoader::Load(char* filename, ID3D11Device* _pd3dDevice, bool invertTexCoords)
{
	...
	if(!binaryInFile.good())
	{
		...
		else
		{
			...
			SimpleVertex2* finalVerts = new SimpleVertex2[meshVertices.size()];
			...
			bd.ByteWidth = sizeof(SimpleVertex2) * meshVertices.size();
			...
			meshData.VBOffset = 0;
			meshData.VBStride = sizeof(SimpleVertex2);
			...
			outbin.write((char*)finalVerts, sizeof(SimpleVertex2) * numMeshVertices);
	...
	else
	{
		...
		SimpleVertex2* finalVerts = new SimpleVertex2[numVertices];
		unsigned short* indices = new unsigned short[numIndices];
		binaryInFile.read((char*)finalVerts, sizeof(SimpleVertex2) * numVertices);
		...
		ZeroMemory(&bd, sizeof(bd));
		bd.Usage = D3D11_USAGE_DEFAULT;
		bd.ByteWidth = sizeof(SimpleVertex2) * numVertices;
		bd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		bd.CPUAccessFlags = 0;
